...
<head>
	...
	<script type="text/javascript">
		...
		{  
			...
			window.open("${contextPath}/Guestbook/GuestbookReplyFormAction.gb?num="+guestbook_no+"&page=${spage}",
					"rForm", "width=800, height=500, resizable = no, scrollbars = no");
		} 
		...
		function openDelForm(guestbook_no)
		{
			...
			window.open("${contextPath}/Guestbook/GuestbookDeleteFormAction.gb?num="+guestbook_no,
		...
		{
			window.name = "parentForm";
			window.open("${contextPath}/Guestbook/GuestbookUpdateFormAction.gb?num="+guestbook_no+"&page=${spage}",
...
<body>
	...
	<br>
	<div style="text-align: center;"><b><font size="5" color="gray">방명록</font></b></div>
	<hr size="1" width="700">
	<br>
...
<div id="wrap">	
	...
	<div id="writeGuestForm">
		<form name="guestbookInfo" 
			  method="post" 
			  action="${contextPath}/Guestbook/GuestbookWriteAction.gb"
			...
			<div id="comment">
				<c:forEach var="guestbook" items="${requestScope.list}">
					...
					<c:if test="${guestbook.guestbook_level > 1}">
						...
						<img src="${contextPath}/img/reply_icon.gif">
					</c:if>
			...
			<!-- 페이지 부분 -->
		 	<div id="pageForm">
					...
					<c:if test="${startPage != 1 && startPage != null}">
						<a href='${contextPath}/Guestbook/GuestbookListAction.gb?page=${startPage-1}'>[ 이전 ]</a>
					</c:if>
					...
					<c:if test="${startPage >= 1}">
						<c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${pageNum != spage}">
								<a href='${contextPath}/Guestbook/GuestbookListAction.gb?page=${pageNum}'>${pageNum}&nbsp;</a>
							</c:if>
						</c:forEach>
					</c:if>
					
				<c:if test="${endPage != maxPage }">
					<a href='${contextPath}/Guestbook/GuestbookListAction.gb?page=${endPage+1 }'>[다음]</a>
				</c:if>
